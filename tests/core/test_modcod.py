...
def assert_decibel_equal(actual, expected, atol=1e-2):
    """
    Assert that two decibel quantities are equal within a tolerance,
    comparing value and unit string.
    ...
    assert actual_unit.startswith("dB") and expected_unit.startswith(
        "dB"
    ), f"Units must be dB-like, got {actual_unit} and {expected_unit}"
    assert (
        abs(actual_val - expected_val) <= atol
    ), f"Values differ: {actual_val} vs {expected_val} (atol={atol})"
    assert (
        actual_unit == expected_unit
        or actual_unit.startswith("dB")
        and expected_unit.startswith("dB")
    ), f"Unit strings differ: {actual_unit} vs {expected_unit}"
...
def test_required_ebno_for_psk_ber_invalid_type():
    ...
    with pytest.raises(TypeError):
        required_ebno_for_psk_ber(
            ber=1e-5, coding_scheme="CC(7,1/2) RS(255,223) I=5"
        )  # Pass float instead of Enum
...
@pytest.mark.parametrize(
    ...
    [
        (
            "CC(7,1/2) RS(223,255) I=5",
            r"k \(255\) cannot be greater than n \(223\)",
        ),  # k > n
        ("CC(7,1/0) RS(255,223) I=5", "denominator cannot be zero"),  # CC n=0
        ("CC(7,1/2) RS(0,0) I=5", r"block size \(n\) cannot be zero"),  # RS n=0
        ("INVALID STRING", "Could not parse CC and RS rates"),
        ("CC(?,?) RS(?,?) I=?", "Could not parse CC and RS rates"),
        (
            "CC(7,1/2) RS(NaN,223) I=5",
            "Error parsing numbers from scheme",
        ),  # Invalid number
    ],
)
