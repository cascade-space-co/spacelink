...
def test_stage_to_from_dict():
    ...
    assert d["label"] == "t"
    assert_allclose(Q_(d["gain_db"], dB), 5 * dB)
    assert_allclose(Q_(d["nf_db"], dB), 3 * dB)
...
def test_cascade_gain():
    s1 = Stage(label="a", gain_db=Q_(10, dB))
    s2 = Stage(label="b", gain_db=Q_(3, dB))
    c = Cascade([s1, s2])
    assert_allclose(c.cascaded_gain_db(), Q_(13, dB))


def test_cascade_noise_figure_and_temperature():
    # Stage1: gf=10db nf=2db, Stage2: gf=3db nf=3db
    s1 = Stage(label="a", gain_db=Q_(10, dB), nf_db=Q_(2, dB))
    s2 = Stage(label="b", gain_db=Q_(10, dB), nf_db=Q_(4, dB))
    s3 = Stage(label="c", gain_db=Q_(10, dB), nf_db=Q_(4, dB))
    c = Cascade([s1, s2, s3])
    ...
    assert_allclose(c.cascaded_gain_db(), Q_(30, dB))


def test_cascade_input_referred_p1db_single_stage():
    s = Stage(label="x", gain_db=Q_(5, dB), p1db_dbm=Q_(20, dBm))
    c = Cascade([s])
...
def test_cascade_len_and_getitem():
    s1 = Stage(label="a", gain_db=Q_(1, dB))
    s2 = Stage(label="b", gain_db=Q_(2, dB))
    c = Cascade([s1, s2])
    assert len(c) == 2
    assert c[0] is s1
    assert c[1] is s2
...
def test_cascade_to_from_dict_and_yaml():
    s1 = Stage(label="a", gain_db=Q_(4, dB), nf_db=Q_(1, dB))
    s2 = Stage(label="b", loss_db=Q_(2, dB), nf_db=Q_(2, dB))
    c = Cascade([s1, s2])
    d = c.to_dict()
    # Round-trip dict
...
def test_cascade_from_yaml_single_stage():
    ...
    expected = Cascade([Stage(label="amp1", gain_db=Q_(10, dB), nf_db=Q_(1.5, dB))])
    assert c == expected
...
def test_cascade_from_yaml_file_simple():
    ...
    c = Cascade.from_dict(data)
    expected = Cascade([Stage(label="simple_amp", gain_db=Q_(10, dB), nf_db=Q_(3, dB))])
    assert c == expected
...
def test_cascade_from_yaml_file_multiple():
    ...
    expected = Cascade(
        [
            Stage(label="amp1", gain_db=Q_(6, dB), nf_db=Q_(1, dB)),
            Stage(label="att1", loss_db=Q_(2, dB), nf_db=Q_(0.5, dB)),
            Stage(label="amp2", gain_db=Q_(4, dB), nf_db=Q_(2, dB)),
        ]
    )
    assert c == expected
...
def test_cascade_vswr_fixture_file():
    ...
    # Stage 1 properties
    assert c[0].label == "s1"
    assert pytest.approx(c[0].gain_db.to(dB).magnitude, abs=1e-6) == 10.0
    # Stage 2 properties: gain and VSWR
    assert c[1].label == "s2"
    assert pytest.approx(c[1].gain_db.to(dB).magnitude, abs=1e-6) == 5.0
